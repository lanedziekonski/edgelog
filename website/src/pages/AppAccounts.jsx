...
export default function AppAccounts() {
  ...
  const [importingAccount, setImportingAccount] = useState(null); // { id, name }
    ...
    <div style={{ maxWidth: 1100 }}>
            ...
            return (
              <motion.div key={acc.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button
                      onClick={() => setImportingAccount({ id: acc.id, name: acc.name })}
                      style={{ padding: 6, borderRadius: 8, background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.4)' }}
      ...
      {importingAccount && (
        <CsvImportModal
          ...
          onImported={() => { reload(); setImportingAccount(null); }}
        />
      )}
